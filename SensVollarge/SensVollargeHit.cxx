...
// -----   Default constructor   --------------
SensVollargeHit::SensVollargeHit()
  : ShipHit()
{
 flag = true;
}
...
SensVollargeHit::SensVollargeHit(Int_t detID, Float_t adc)
  : ShipHit(detID,adc)
{
 ft = -1;
 flag = true;
...
// ----------------------------------------------
TVector3 SensVollargeHit::GetXYZ()
{
...
Double_t SensVollargeHit::GetX()
{ TVector3 pos = GetXYZ();
  return pos.X();
}


Double_t SensVollargeHit::GetY()
{ TVector3 pos = GetXYZ();
  return pos.Y();
}


Double_t SensVollargeHit::GetZ()
{ TVector3 pos = GetXYZ();
  return pos.Z();
}


TGeoNode* SensVollargeHit::GetNode()
{
   TGeoNode* node=0;
   TGeoNavigator* nav = gGeoManager->GetCurrentNavigator();
   ...
   if (fDetectorID>999999){ // plastic scintillator ABBCCCD
    ...
    Int_t SensVollargeNr = fDetectorID;
...
// -----   Public method Print   -----------------------
void SensVollargeHit::Print(Int_t detID) const
{ 
  cout << "-I- SensVollargeHit: SensVollarge hit " << " in detector " << fDetectorID << endl;
  cout << "  ADC " << fdigi << " ns" << endl;
}


// -----------------------------------------------------
ClassImp(SensVollargeHit)
