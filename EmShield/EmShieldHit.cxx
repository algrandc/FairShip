...
EmShieldHit::EmShieldHit(Int_t detID, Float_t adc)
  : ShipHit(detID,adc)
{
 ft = -1;
 flag = true;
...
Double_t EmShieldHit::GetX()
{ TVector3 pos = GetXYZ();
  return pos.X();
}


Double_t EmShieldHit::GetY()
{ TVector3 pos = GetXYZ();
  return pos.Y();
}


Double_t EmShieldHit::GetZ()
{ TVector3 pos = GetXYZ();
  return pos.Z();
}


TGeoNode* EmShieldHit::GetNode()
{
   TGeoNode* node=0;
   TGeoNavigator* nav = gGeoManager->GetCurrentNavigator();
...
// -----   Public method Print   -----------------------
void EmShieldHit::Print(Int_t detID) const
{ 
  cout << "-I- EmShieldHit: EmShield hit " << " in detector " << fDetectorID << endl;
  cout << "  ADC " << fdigi << " ns" << endl;
}


// -----------------------------------------------------
ClassImp(EmShieldHit)
